# 🎨 数字原生时代泛在媒体艺术 | Ubiquitous-Media Art (UMA)

数字原生时代泛在媒体艺术 | Ubiquitous-Media Art 数字原生时代的艺术创作框架，拥抱泛在计算、人工智能与分布式智能，重新定义创作者、作品与体验的关系 | An art creation framework for the digital native era, embracing ubiquitous computing, artificial intelligence, and distributed intelligence, redefining the relationships between creators, works, and experiences

## 📜 项目愿景 | Project Vision
**中文**：一个面向数字原生时代的艺术创作框架，拥抱泛在计算、人工智能与分布式智能，重新定义创作者、作品与体验的关系。

**English**：An art creation framework for the digital native era, embracing ubiquitous computing, artificial intelligence, and distributed intelligence, redefining the relationships between creators, works, and experiences.

## 🏛️ 核心概念体系 | Core Concept System

### 🧠 哲学基础概念 (UMA-000 ~ UMA-099)

| 概念编号 | 概念名称 | 简短描述 | 详细链接 |
|---------|---------|---------|----------|
| **UMA-001** | [泛在性 (Ubiquity)](./CONCEPTS/UMA-001-Ubiquity.md) | 计算、网络、智能无处不在，艺术资源从稀缺走向丰裕 | [查看详情](./CONCEPTS/UMA-001-Ubiquity.md) |
| **UMA-002** | [向前逃逸 (Forward Escape)](./CONCEPTS/UMA-002-Forward-Escape.md) | 面对技术奇点，不是后退而是向更复杂的未来跃进 | [查看详情](./CONCEPTS/UMA-002-Forward-Escape.md) |
| **UMA-003** | [扩散作者性 (Diffused Authorship)](./CONCEPTS/UMA-003-Diffused-Authorship.md) | 在人与智能网络的共生中，重新界定创造的主体性 | [查看详情](./CONCEPTS/UMA-003-Diffused-Authorship.md) |
| **UMA-004** | [终极震撼 (Ultimate Spectacle)](./CONCEPTS/UMA-004-Ultimate-Spectacle.md) | 追求超越日常经验的感官与认知冲击 | [查看详情](./CONCEPTS/UMA-004-Ultimate-Spectacle.md) |

### 🎨 创作实践概念 (UMA-200 ~ UMA-299)

| 概念编号 | 概念名称 | 简短描述 | 详细链接 |
|---------|---------|---------|----------|
| **UMA-201** | [创作方法论 (Creation Methodology)](./CONCEPTS/UMA-201-Creation-Methodology.md) | 泛在媒体艺术的系统性创作方法，强调过程的开放性、迭代性和技术与艺术的深度融合 | [查看详情](./CONCEPTS/UMA-201-Creation-Methodology.md) |

### 📐 美学体系概念 (UMA-300 ~ UMA-399)

| 概念编号 | 概念名称 | 简短描述 | 详细链接 |
|---------|---------|---------|----------|
| **UMA-301** | [终极震撼美学 (Ultimate Spectacle Aesthetics)](./CONCEPTS/UMA-301-Ultimate-Spectacle-Aesthetics.md) | 通过技术手段创造超越日常经验的美学体验，实现认知超越和精神升华 | [查看详情](./CONCEPTS/UMA-301-Ultimate-Spectacle-Aesthetics.md) |

### 🔄 作品形态概念 (UMA-400 ~ UMA-499)

| 概念编号 | 概念名称 | 简短描述 | 详细链接 |
|---------|---------|---------|----------|
| **UMA-401** | [实时演变架构 (Real-Time Evolution Architecture)](./CONCEPTS/UMA-401-Real-Time-Evolution-Architecture.md) | 作品具有生命般的特性，能够实时响应环境变化和观众互动，在持续的演变中呈现不同的状态 | [查看详情](./CONCEPTS/UMA-401-Real-Time-Evolution-Architecture.md) |

## 📁 项目结构 | Project Structure

### 🧠 概念与理论 | Concepts & Theory
- **/CONCEPTS** - 概念定义库（编号UMA-XXX）
- **/MANIFESTO.md** - UMA宣言与哲学基础

### 🎯 创作与实践 | Creation & Practice
- **/PRACTICES** - 创作实践与方法论
- **/PROJECTS** - 示例项目与完整案例
- **/EXAMPLES** - 最小可运行示例

### 🛠️ 工具与资源 | Tools & Resources
- **/TOOLS** - 工具栈与技术基础设施
- **/TEMPLATES** - 项目模板与脚手架

### 🌍 社区与展示 | Community & Exhibition
- **/COMMUNITY** - 社区讨论与协作记录
- **/GALLERY** - 作品展示与元数据

## 🚀 快速开始 | Quick Start

1. **阅读宣言** - 阅读 [MANIFESTO.md](./MANIFESTO.md) 了解哲学基础
2. **学习概念** - 浏览 [CONCEPTS/](./CONCEPTS/) 学习核心概念
3. **运行示例** - 查看 [EXAMPLES/](./EXAMPLES/) 运行第一个UMA项目
4. **开始创作** - 使用 [TEMPLATES/](./TEMPLATES/) 开始你的创作

## 📄 许可证 | License

本项目采用双重许可证模式：

### 📚 概念与文档 | Concepts & Documents
**Creative Commons Attribution-ShareAlike 4.0 International (CC BY-SA 4.0)**

### 💻 代码与软件 | Code & Software
**MIT License**

## 🌐 社区 | Community
- **GitHub Issues**: 问题反馈与功能建议
- **GitHub Discussions**: 概念讨论与社区交流

## 📞 联系我们 | Contact
- **项目仓库**: [https://github.com/ewanqian/ubiquitous-media-art](https://github.com/ewanqian/ubiquitous-media-art)

---

<div style="text-align: center; padding: 20px; background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); color: white; border-radius: 12px; margin-top: 30px;">
  <h3>🌟 加入泛在媒体艺术的未来 🌟</h3>
  <p>拥抱技术，创造未来，重新定义艺术的边界</p>
  <p>Embrace technology, create the future, redefine the boundaries of art</p>
</div>